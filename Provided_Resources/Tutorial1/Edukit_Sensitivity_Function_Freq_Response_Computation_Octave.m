...
%                        STMicroelectronics
%
%            Edukit_Sensitivity_Function_Frequency_Computation_Octave.m
%
%                         Octave Version 2.0
%                         November 11, 2021
%
...
[file,path] = uigetfile('*.mat', 'Select Input Data with Sensitivity Function Data');
if isequal(file,0)
    ...
    disp(['User selected ', fullfile(path,file)]);
      ...
      h=msgbox('Please Standby for Data Load');
...
selpath = uigetdir(pwd, 'Select Output Data Directory for Figures');
...
% Start signal acquisition and averaging
      ...
      h=msgbox('Please Standby for Data Processing');
      delete(findobj(h,'string','OK'));
      delete(findobj(h,'style','frame'));
...
while i < length(rotor_command)
    if rotor_command(i - 1) < 0 && rotor_command(i) > rotor_command(i - 1)
        ...
        if Display_Cycles == 1
            ...
            figure('units', 'normalized', 'Position',[startx starty endx endy]);
            ax = plotyy(sample_start(j):sample_end(j),pendulum_angle(sample_start(j):sample_end(j)),sample_start(j):sample_end(j),rotor_angle(sample_start(j):sample_end(j)));
            xlabel('Samples')
            ylabel(ax(1),'Pendulum Angle (degrees)');
            ylabel(ax(2),'Rotor Angle (degrees)');
            legend('Pendulum Angle', 'Rotor Angle')
            next_plot; if exit_state == 1 return; end;
            
            figure('units', 'normalized', 'Position',[startx starty endx endy]);
            ax = plotyy(sample_start(j):sample_end(j), motor_position_target(sample_start(j):sample_end(j)),sample_start(j):sample_end(j), rotor_command(sample_start(j):sample_end(j)));
            xlabel('Samples')
            ylabel(ax(1), 'Pendulum Angle (degrees)');
            ylabel(ax(2), 'Rotor Angle (degrees)');
...
for i = 1:NFFT/2
    if abs(Rotor_Angle_Avg_Peak) < abs(rotor_angle_norm(i)) && strcmp(data_display, "Noise Sensitivity Function") == 1
        Rotor_Angle_Avg_Peak = rotor_angle_norm(i);
        Rotor_Angle_Avg_Peak_Index = i;
    end
    if abs(Rotor_Angle_Avg_Peak) < abs(rotor_angle_norm(i)) && rotor_p < 0 && strcmp(data_display, "Noise Sensitivity Function") == 0
        Rotor_Angle_Avg_Peak = rotor_angle_norm(i);
        Rotor_Angle_Avg_Peak_Index = i;
    end
    if abs(Rotor_Angle_Avg_Peak) < abs(rotor_angle_norm(i)) && rotor_p > 0 && strcmp(data_display, "Noise Sensitivity Function") == 0
...
if strcmp(data_display, "Step Response Rotor Angle (degrees)") == 1
    Settling_Time_Tolerance = 0.02;
end

if strcmp(data_display, "Disturbance Sensitivity Function") == 1 && pend_p < 0
    Settling_Time_Tolerance = 0.02;
end
...
if strcmp(data_display, "Noise Sensitivity Function") == 1 && pend_p > 0
...
if strcmp(data_display, "Step Response Rotor Angle (degrees)") == 0
...
if strcmp(data_display, "Disturbance Sensitivity Function") == 1
...
if  strcmp(data_display, "Sensitivity Function") == 1
...
if strcmp(data_display, "Noise Sensitivity Function") == 1
...
for i = 1:NFFT/2
    if abs(Pendulum_Angle_Avg_Peak) < abs(pendulum_angle_norm(i))
...
figure('units', 'normalized', 'Position',[startx starty endx endy]);
semilogx(W(Plot_Start:Plot_Length - 1),20*log10(Mag(Plot_Start:Plot_Length - 1)),'*')
if strcmp(data_display, "Noise Sensitivity Function") == 1
    xlim([0.1 1000])
end
if strcmp(data_display, "Noise Sensitivity Function") == 0
    xlim([0.1 100])
end
...
if strcmp(data_display, "Sensitivity Function") == 1
    ylabel('Sensitivity Function (dB)');
        title_str = sprintf('Sensitivity Function (dB)');
    title(title_str);
end
...
figure('units', 'normalized', 'Position',[startx starty endx endy]);
semilogx(W(Plot_Start:Plot_Length - 1),20*log10(Mag(Plot_Start:Plot_Length - 1)),'*k')
if strcmp(data_display, "Noise Sensitivity Function") == 1
    xlim([0.1 1000]);
    xlabel('Frequency (rad/sec)');
    ylabel('Noise Rejection Sensitivity Function (dB)');
...
save_filename = fullfile(selpath,fig_save_filename);
savefig(gcf, save_filename);
last_plot; if exit_state == 1 return; end;
